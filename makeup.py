...
class makeup(object):
    ...
    def apply_eyebrow(self, landmark):
        ...
        # rescale
        right_eye = cv2.imread('./data/right_eyebrow.png')
        ...
        # apply eyebrow
        for c in range(0, 3):
            self.im_copy[y1:y2, x1:x2, c] = (alpha_s * right_eye[:, :, c] + alpha_l * self.im_copy[y1:y2, x1:x2, c])
        ...
        # apply eyebrow
        for c in range(0, 3):
            self.im_copy[y1:y2, x1:x2, c] = (alpha_s * left_eye[:, :, c] + alpha_l * self.im_copy[y1:y2, x1:x2, c])
        return self.im_copy
    ...
    def apply_makeup(self, landmarks,r,g,b):
        self.im_copy = self.apply_lipstick(landmarks, r, g, b)
    ...
    def apply_makeup_all(self, landmarks):
        # self.im_copy = self.apply_lipstick(landmarks, 177, 55, 75)
        # self.im_copy = self.apply_foundation(landmarks)
        # self.im_copy = self.apply_blush(landmarks, 223., 91., 111.)
        # self.im_copy = self.apply_eyebrow(landmarks)
        # self.im_copy = self.apply_liner(landmarks)
